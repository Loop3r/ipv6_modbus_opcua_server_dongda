...
int Parse_Config_File(){
    ...
    if(-1 == Get_Config_String("IMOS.conf", "ALL", "devnum", devnum)){
        exit(EXIT_FAILURE);
    }
    if(-1 == Get_Config_String("IMOS.conf", "ALL", "timeout", timeout)){
        exit(EXIT_FAILURE);
    }
    if(-1 == Get_Config_String("IMOS.conf", "ALL", "delay", delay)){
        exit(EXIT_FAILURE);
    }
